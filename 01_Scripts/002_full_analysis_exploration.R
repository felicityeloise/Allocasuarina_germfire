...
rec_m5l <- glmer(Proportion_seedlings ~ r_TSF * r_Temp + (1|Location/Transect), family = binomial, data = lit_transects)
summary(rec_m5l)
...
rsap_m3l <- glmer(Proportion_saplings ~ r_TSF * r_FPC + (1|Location/Transect), family = binomial, data = lit_transects, control = glmerControl(optCtrl=list(maxfun=30)))
...
rsap_m4t <-  glmer(Proportion_saplings ~ r_TSF * r_Precip + (1|Location/Transect), family = binomial, data = tor_transects, control = glmerControl(optCtrl=list(maxfun=30)))
...
rsap_m4l <-  glmer(Proportion_saplings ~ r_TSF * r_Precip + (1|Location/Transect), family = binomial, data = lit_transects, control = glmerControl(optCtrl=list(maxfun=30)))
...
rsap_m5t <-  glmer(Proportion_saplings ~ r_TSF * r_Temp + (1|Location/Transect), family = binomial, data = tor_transects, control = glmerControl(optCtrl=list(maxfun=30)))
...
rsap_m5l <-  glmer(Proportion_saplings ~ r_TSF * r_Temp + (1|Location/Transect), family = binomial, data = lit_transects, control = glmerControl(optCtrl=list(maxfun=30)))
...
aictab(rsap_aic_t) # Model 1 and 2 are better than the null but the null model is ranked within delta AICc <2.
...
rsap_td_aic$LL[5] <- AICc(rsap_g4t)
rsap_td_aic$Model[5] <- "m4"
rsap_td_aic$LL[5] <- logLik(rsap_g4t)
rsap_td_aic$LL[6] <- AICc(rsap_g5t)
...
rsap_td_aic  # Model 1 and 2 are good but null model ranked within delta AICc <2. 
...
rsap_ts_aic  #Model 1 is best but null model ranked within AICc <2.
...
rsap_tsc_aic  #Model 1 and 2 are best but null model is ranked within AICc <2.
...
nsap_gm1  <- data.frame(TSF = seq(min(tor_transects$TSF), max(tor_transects$TSF), length = 50))
nsap_gm2l <-  expand.grid(TSF = min(tor_transects$TSF),
                          Latitude = seq(min(tor_transects$Latitude), max(tor_transects$Latitude), length = 50))
nsap_gm2a <-  expand.grid(TSF = mean(tor_transects$TSF),
                          Latitude = seq(min(tor_transects$Latitude), max(tor_transects$Latitude), length = 50))
nsap_gm2h <-  expand.grid(TSF = max(tor_transects$TSF),
                          Latitude = seq(min(tor_transects$Latitude), max(tor_transects$Latitude), length = 50))

# GLMER 
nsap_m1  <- data.frame(r_TSF = seq(min(tor_transects$r_TSF), max(tor_transects$r_TSF), length = 50))
nsap_m2l <-  expand.grid(r_TSF = min(tor_transects$r_TSF),
                          r_Latitude = seq(min(tor_transects$r_Latitude), max(tor_transects$r_Latitude), length = 50))
nsap_m2a <-  expand.grid(r_TSF = mean(tor_transects$r_TSF),
                          r_Latitude = seq(min(tor_transects$r_Latitude), max(tor_transects$r_Latitude), length = 50))
nsap_m2h <-  expand.grid(r_TSF = max(tor_transects$r_TSF),
                          r_Latitude = seq(min(tor_transects$r_Latitude), max(tor_transects$r_Latitude), length = 50))


psap_m1 <- predictSE(rsap_m1t, newdata = nsap_m1, se.fit = T, type = 'link')
nsap_m1$fit.link <- psap_m1$fit
nsap_m1$se.link <- psap_m1$se.fit
nsap_m1$lci.link <- nsap_m1$fit.link - (nsap_m1$se.link * 1.96)
nsap_m1$uci.link <- nsap_m1$fit.link + (nsap_m1$se.link * 1.96)
nsap_m1$fit <- invlogit(nsap_m1$fit.link)
nsap_m1$se <- invlogit(nsap_m1$se.link)
nsap_m1$lci <- invlogit(nsap_m1$lci.link)
nsap_m1$uci <- invlogit(nsap_m1$uci.link)

psap_m2l <- predictSE(rsap_m2t, newdata = nsap_m2l, se.fit = T, type = 'link')
nsap_m2l$fit.link <- psap_m2l$fit
nsap_m2l$se.link <- psap_m2l$se.fit
nsap_m2l$lci.link <- nsap_m2l$fit.link - (nsap_m2l$se.link * 1.96)
nsap_m2l$uci.link <- nsap_m2l$fit.link + (nsap_m2l$se.link * 1.96)
nsap_m2l$fit <- invlogit(nsap_m2l$fit.link)
nsap_m2l$se <- invlogit(nsap_m2l$se.link)
nsap_m2l$lci <- invlogit(nsap_m2l$lci.link)
nsap_m2l$uci <- invlogit(nsap_m2l$uci.link)

psap_m2a <- predictSE(rsap_m2t, newdata = nsap_m2a, se.fit = T, type = 'link')
nsap_m2a$fit.link <- psap_m2a$fit
nsap_m2a$se.link <- psap_m2a$se.fit
nsap_m2a$lci.link <- nsap_m2a$fit.link - (nsap_m2a$se.link * 1.96)
nsap_m2a$uci.link <- nsap_m2a$fit.link + (nsap_m2a$se.link * 1.96)
nsap_m2a$fit <- invlogit(nsap_m2a$fit.link)
nsap_m2a$se <- invlogit(nsap_m2a$se.link)
nsap_m2a$lci <- invlogit(nsap_m2a$lci.link)
nsap_m2a$uci <- invlogit(nsap_m2a$uci.link)

psap_m2h <- predictSE(rsap_m2t, newdata = nsap_m2h, se.fit = T, type = 'link')
nsap_m2h$fit.link <- psap_m2h$fit
nsap_m2h$se.link <- psap_m2h$se.fit
nsap_m2h$lci.link <- nsap_m2h$fit.link - (nsap_m2h$se.link * 1.96)
nsap_m2h$uci.link <- nsap_m2h$fit.link + (nsap_m2h$se.link * 1.96)
nsap_m2h$fit <- invlogit(nsap_m2h$fit.link)
nsap_m2h$se <- invlogit(nsap_m2h$se.link)
nsap_m2h$lci <- invlogit(nsap_m2h$lci.link)
nsap_m2h$uci <- invlogit(nsap_m2h$uci.link)


# GAM default
nsap_g1 <- nsap_gm1
psap_g1 <- predict(rsap_g1t, newdata = nsap_gm1, se.fit = T, type = 'response')
nsap_g1$fit.link <- psap_g1$fit
nsap_g1$se.link <- psap_g1$se.fit
nsap_g1$lci.link <- nsap_g1$fit.link - (nsap_g1$se.link * 1.96)
nsap_g1$uci.link <- nsap_g1$fit.link + (nsap_g1$se.link * 1.96)
nsap_g1$fit <- invlogit(nsap_g1$fit.link)
nsap_g1$se <- invlogit(nsap_g1$se.link)
nsap_g1$lci <- invlogit(nsap_g1$lci.link)
nsap_g1$uci <- invlogit(nsap_g1$uci.link)

nsap_g2l <- nsap_gm2l
psap_g2l <- predict(rsap_g2t, newdata = nsap_gm2l, se.fit = T, type = 'response')
nsap_g2l$fit.link <- psap_g2l$fit
nsap_g2l$se.link <- psap_g2l$se.fit
nsap_g2l$lci.link <- nsap_g2l$fit.link - (nsap_g2l$se.link * 1.96)
nsap_g2l$uci.link <- nsap_g2l$fit.link + (nsap_g2l$se.link * 1.96)
nsap_g2l$fit <- invlogit(nsap_g2l$fit.link)
nsap_g2l$se <- invlogit(nsap_g2l$se.link)
nsap_g2l$lci <- invlogit(nsap_g2l$lci.link)
nsap_g2l$uci <- invlogit(nsap_g2l$uci.link)


nsap_g2a <- nsap_gm2a
psap_g2a <- predict(rsap_g2t, newdata = nsap_gm2a, se.fit = T, type = 'response')
nsap_g2a$fit.link <- psap_g2a$fit
nsap_g2a$se.link <- psap_g2a$se.fit
nsap_g2a$lci.link <- nsap_g2a$fit.link - (nsap_g2a$se.link * 1.96)
nsap_g2a$uci.link <- nsap_g2a$fit.link + (nsap_g2a$se.link * 1.96)
nsap_g2a$fit <- invlogit(nsap_g2a$fit.link)
nsap_g2a$se <- invlogit(nsap_g2a$se.link)
nsap_g2a$lci <- invlogit(nsap_g2a$lci.link)
nsap_g2a$uci <- invlogit(nsap_g2a$uci.link)


nsap_g2h <- nsap_gm2h
psap_g2h <- predict(rsap_g2t, newdata = nsap_gm2h, se.fit = T, type = 'response')
nsap_g2h$fit.link <- psap_g2h$fit
nsap_g2h$se.link <- psap_g2h$se.fit
nsap_g2h$lci.link <- nsap_g2h$fit.link - (nsap_g2h$se.link * 1.96)
nsap_g2h$uci.link <- nsap_g2h$fit.link + (nsap_g2h$se.link * 1.96)
nsap_g2h$fit <- invlogit(nsap_g2h$fit.link)
nsap_g2h$se <- invlogit(nsap_g2h$se.link)
nsap_g2h$lci <- invlogit(nsap_g2h$lci.link)
nsap_g2h$uci <- invlogit(nsap_g2h$uci.link)


# GAM bs = 'cr'
nsap_g1.1 <- nsap_gm1
psap_g1.1 <- predict(rsap_g1.1t, newdata = nsap_gm1, se.fit = T, type = 'response')
nsap_g1.1$fit.link <- psap_g1.1$fit
nsap_g1.1$se.link <- psap_g1.1$se.fit
nsap_g1.1$lci.link <- nsap_g1.1$fit.link - (nsap_g1.1$se.link * 1.96)
nsap_g1.1$uci.link <- nsap_g1.1$fit.link + (nsap_g1.1$se.link * 1.96)
nsap_g1.1$fit <- invlogit(nsap_g1.1$fit.link)
nsap_g1.1$se <- invlogit(nsap_g1.1$se.link)
nsap_g1.1$lci <- invlogit(nsap_g1.1$lci.link)
nsap_g1.1$uci <- invlogit(nsap_g1.1$uci.link)

nsap_g2.2l <- nsap_gm2l
psap_g2.2l <- predict(rsap_g2.2t, newdata = nsap_gm2l, se.fit = T, type = 'response')
nsap_g2.2l$fit.link <- psap_g2.2l$fit
nsap_g2.2l$se.link <- psap_g2.2l$se.fit
nsap_g2.2l$lci.link <- nsap_g2.2l$fit.link - (nsap_g2.2l$se.link * 1.96)
nsap_g2.2l$uci.link <- nsap_g2.2l$fit.link + (nsap_g2.2l$se.link * 1.96)
nsap_g2.2l$fit <- invlogit(nsap_g2.2l$fit.link)
nsap_g2.2l$se <- invlogit(nsap_g2.2l$se.link)
nsap_g2.2l$lci <- invlogit(nsap_g2.2l$lci.link)
nsap_g2.2l$uci <- invlogit(nsap_g2.2l$uci.link)


nsap_g2.2a <- nsap_gm2a
psap_g2.2a <- predict(rsap_g2.2t, newdata = nsap_gm2a, se.fit = T, type = 'response')
nsap_g2.2a$fit.link <- psap_g2.2a$fit
nsap_g2.2a$se.link <- psap_g2.2a$se.fit
nsap_g2.2a$lci.link <- nsap_g2.2a$fit.link - (nsap_g2.2a$se.link * 1.96)
nsap_g2.2a$uci.link <- nsap_g2.2a$fit.link + (nsap_g2.2a$se.link * 1.96)
nsap_g2.2a$fit <- invlogit(nsap_g2.2a$fit.link)
nsap_g2.2a$se <- invlogit(nsap_g2.2a$se.link)
nsap_g2.2a$lci <- invlogit(nsap_g2.2a$lci.link)
nsap_g2.2a$uci <- invlogit(nsap_g2.2a$uci.link)


nsap_g2.2h <- nsap_gm2h
psap_g2.2h <- predict(rsap_g2.2t, newdata = nsap_gm2h, se.fit = T, type = 'response')
nsap_g2.2h$fit.link <- psap_g2.2h$fit
nsap_g2.2h$se.link <- psap_g2.2h$se.fit
nsap_g2.2h$lci.link <- nsap_g2.2h$fit.link - (nsap_g2.2h$se.link * 1.96)
nsap_g2.2h$uci.link <- nsap_g2.2h$fit.link + (nsap_g2.2h$se.link * 1.96)
nsap_g2.2h$fit <- invlogit(nsap_g2.2h$fit.link)
nsap_g2.2h$se <- invlogit(nsap_g2.2h$se.link)
nsap_g2.2h$lci <- invlogit(nsap_g2.2h$lci.link)
nsap_g2.2h$uci <- invlogit(nsap_g2.2h$uci.link)



# Plot predictions proportion saplings -----
# GLMER
plot(nsap_m1$r_TSF, nsap_m1$fit, type = 'l', xlim = c(-1, 1.9), las = 1, xlab = "", ylab = "", ylim = c(0,1), cex.axis = 1.4, xaxt = "n")
axis(side = 1, at = seq(-0.9, 1.86, 0.115), labels = seq(1,25,1), cex.axis = 1.4)
mtext(side = 1, expression(bold("Time since fire")), line = 4, cex = 1.5)
mtext(side = 2, expression(bold("Proportion saplings")), line = 3, cex = 1.5)
mtext(paste("AICc = ", round(AICc(rsap_m1t ),3), sep = ""), line = -1.5, cex = 1.2)
pg.ci(x = 'r_TSF', data = "nsap_m1", colour = rgb(0,0,0,0.1), lower = 'lci', upper = 'uci')


plot(nsap_m2l$r_Latitude, nsap_m2l$fit, type = 'l', las = 1, xlab = "", ylab = "", ylim = c(0,1), cex.axis = 1.4, col = "blue", xaxt = "n")
axis(side = 1, at = seq(-1.364717, 1.103191, 0.25), labels = round(seq(-28.28369, -27.31778, 0.1), 1))
mtext(side = 1, expression(bold("Latitude")), line = 4, cex = 1.5)
mtext(side = 2, expression(bold("Proportion saplings")), line = 3, cex = 1.5)
mtext(paste("AICc = ", round(AICc(rsap_m2t ),3), sep = ""), line = -1.5, cex = 1.2)
pg.ci(x = 'r_Latitude', data = "nsap_m2l", colour = rgb(0,0,1,0.1), lower = 'lci', upper = 'uci')
lines(nsap_m2a$r_Latitude, nsap_m2a$fit, type = "l", col = "black")
pg.ci(x = 'r_Latitude', data = "nsap_m2a", colour = rgb(0,0,0,0.1), lower = 'lci', upper = 'uci')
lines(nsap_m2h$r_Latitude, nsap_m2h$fit, type = "l", col = "red")
pg.ci(x = 'r_Latitude', data = "nsap_m2h", colour = rgb(1,0,0,0.1), lower = 'lci', upper = 'uci')



# GAM comparisons
par(mfrow = c(2,2))
plot(nsap_g1$TSF, nsap_g1$fit, type = 'l', las = 1, xlab = "", ylab = "", ylim = c(0,1), cex.axis = 1.4)
mtext(side = 1, expression(bold("Time since fire")), line = 4, cex = 1.5)
mtext(side = 2, expression(bold("Proportion saplings")), line = 3, cex = 1.5)
mtext(paste("AICc = ", round(AICc(rsap_g1t ),3), sep = ""), line = -1.5, cex = 1.2)
pg.ci(x = 'TSF', data = "nsap_g1", colour = rgb(0,0,0,0.1), lower = 'lci', upper = 'uci')
mtext(expression(bold("(a) GAM s(bs = 'tp')")), cex = 2)

plot(nsap_g2l$Latitude, nsap_g2l$fit, type = 'l', las = 1, xlab = "", ylab = "", ylim = c(0,1), cex.axis = 1.4, col = "blue")
mtext(side = 1, expression(bold("Latitude")), line = 4, cex = 1.5)
mtext(side = 2, expression(bold("Proportion saplings")), line = 3, cex = 1.5)
mtext(paste("AICc = ", round(AICc(rsap_g2t ),3), sep = ""), line = -1.5, cex = 1.2)
pg.ci(x = 'Latitude', data = "nsap_g2l", colour = rgb(0,0,1,0.1), lower = 'lci', upper = 'uci')
lines(nsap_g2a$Latitude, nsap_g2a$fit, type = "l", col = "black")
pg.ci(x = 'Latitude', data = "nsap_g2a", colour = rgb(0,0,0,0.1), lower = 'lci', upper = 'uci')
lines(nsap_g2h$Latitude, nsap_g2h$fit, type = "l", col = "red")
pg.ci(x = 'Latitude', data = "nsap_g2h", colour = rgb(1,0,0,0.1), lower = 'lci', upper = 'uci')
mtext(expression(bold("(b) GAM s(bs = 'tp')")), cex = 2)


plot(nsap_g1.1$TSF, nsap_g1.1$fit, type = 'l', las = 1, xlab = "", ylab = "", ylim = c(0,1), cex.axis = 1.4)
mtext(side = 1, expression(bold("Time since fire")), line = 4, cex = 1.5)
mtext(side = 2, expression(bold("Proportion saplings")), line = 3, cex = 1.5)
mtext(paste("AICc = ", round(AICc(rsap_g1.1t ),3), sep = ""), line = -1.5, cex = 1.2)
pg.ci(x = 'TSF', data = "nsap_g1.1", colour = rgb(0,0,0,0.1), lower = 'lci', upper = 'uci')
mtext(expression(bold("(a) GAM ti(bs = 'cr')")), cex = 2)

plot(nsap_g2.2l$Latitude, nsap_g2.2l$fit, type = 'l', las = 1, xlab = "", ylab = "", ylim = c(0,1), cex.axis = 1.4, col = "blue")
mtext(side = 1, expression(bold("Latitude")), line = 4, cex = 1.5)
mtext(side = 2, expression(bold("Proportion saplings")), line = 3, cex = 1.5)
mtext(paste("AICc = ", round(AICc(rsap_g2.2t ),3), sep = ""), line = -1.5, cex = 1.2)
pg.ci(x = 'Latitude', data = "nsap_g2.2l", colour = rgb(0,0,1,0.1), lower = 'lci', upper = 'uci')
lines(nsap_g2.2a$Latitude, nsap_g2.2a$fit, type = "l", col = "black")
pg.ci(x = 'Latitude', data = "nsap_g2.2a", colour = rgb(0,0,0,0.1), lower = 'lci', upper = 'uci')
lines(nsap_g2.2h$Latitude, nsap_g2.2h$fit, type = "l", col = "red")
pg.ci(x = 'Latitude', data = "nsap_g2.2h", colour = rgb(1,0,0,0.1), lower = 'lci', upper = 'uci')
mtext(expression(bold("(b) GAM ti(bs = 'cr')")), cex = 2)


# Default k GAMs look the same, choose the bs method with lowest AICc, in this case would recommend bs = 'tp'. 
dev.new(width = 16, height = 12, noRStudioGD = T, dpi = 300)
...
mtext(paste("AICc = ", round(AICc(rsap_m1t ),3), sep = ""), line = -1.5, cex = 1.2)
pg.ci(x = 'r_TSF', data = "nsap_m1", colour = rgb(0,0,0,0.1), lower = 'lci', upper = 'uci')
mtext(expression(bold("GLMER")), cex = 2, adj = 1.3)
...
par(xpd = NA)
legend(x = 1.2, y = 1, legend = c("1 year", "9.5 years", "25 years"), col = c("blue", "black", 'red'), title = expression(bold("Time since \n fire")), lty = 1, lwd = 2, cex = 1.8, bty = "n")
par(xpd = F)
...
mtext(paste("AICc = ", round(AICc(rsap_g1t ),3), sep = ""), line = -1.5, cex = 1.2)
pg.ci(x = 'TSF', data = "nsap_g1", colour = rgb(0,0,0,0.1), lower = 'lci', upper = 'uci')
mtext(expression(bold("GAM - bs = 'tp'")), cex = 2, adj = 1.3)
...
recruit_m4l <- glmer(Proportion_recruits ~ r_TSF * r_Precip + (1|Location/Transect), family = binomial, data = lit_transects, control = glmerControl(optCtrl=list(maxfun=30)))
summary(recruit_m4l) # Not a very good model
...
recruit_m5t <- glmer(Proportion_recruits ~ r_TSF * r_Precip + (1|Location/Transect), family = binomial, data = tor_transects)
recruit_g5t <- gam(Proportion_recruits ~ s(TSF) + s(Precip) + ti(Precip, by = TSF), random = ~ (1 | Transect/Location), family = binomial, data = tor_transects, method = 'ML')
par(mfrow = c(2,2)); gam.check(recruit_g5t)
plot(recruit_g5t)
recruit_g5.1t <- gam(Proportion_recruits ~ s(TSF) + s(Precip) + ti(Precip, by = TSF, bs = 'tp', k = 10), random = ~ (1 | Transect/Location), family = binomial, data = tor_transects, method = 'ML')
par(mfrow = c(2,2)); gam.check(recruit_g5.1t)
plot(recruit_g5.1t)
recruit_g5.2t <- gam(Proportion_recruits ~ s(TSF, bs = 'cr', k = 5) + s(Precip, bs = 'cr', k = 5) + ti(Precip, by = TSF, bs = 'cr', k = 5), random = ~ (1 | Transect/Location), family = binomial, data = tor_transects, method = 'ML')
par(mfrow = c(2,2)); gam.check(recruit_g5.1t)
plot(recruit_g5.2t)


recruit_m5l <- glmer(Proportion_recruits ~ r_TSF * r_Precip + (1|Location/Transect), family = binomial, data = lit_transects, control = glmerControl(optCtrl=list(maxfun=30)))
summary(recruit_m5l) # Not a very good model
recruit_g5l <- gam(Proportion_recruits ~ s(TSF, k = 4) + s(Precip, k = 4) + ti(Precip, by = TSF), random = ~ (1 | Transect/Location), family = binomial, data = lit_transects, method = 'ML')
par(mfrow = c(2,2)); gam.check(recruit_g5l)
plot(recruit_g5l)
recruit_g5.1l <- gam(Proportion_recruits ~ s(TSF, k = 4) + s(Precip, k = 4) + ti(Precip, by = TSF, bs = 'tp', k = 7), random = ~ (1 | Transect/Location), family = binomial, data = lit_transects, method = 'ML')
par(mfrow = c(2,2)); gam.check(recruit_g5.1l)
plot(recruit_g5.1l)
recruit_g5.2l <- gam(Proportion_recruits ~ s(TSF, bs = 'cr', k = 4) + s(Precip, bs = 'cr', k = 5) + ti(Precip, by = TSF, bs = 'cr', k = 5), random = ~ (1 | Transect/Location), family = binomial, data = lit_transects, method = 'ML')
...
aictab(recruit_aic_t) # Model 2 is best with no model ranked within delta AICc <2.

recruit_aic_l <- list(recruit_lnull, recruit_m1l, recruit_m2.1l, recruit_m3l, recruit_m4l, recruit_m5t)
aictab(recruit_aic_l) # Null model is best
...
recruit_td_aic  # Model 2 is best and model 1 is ranked within delta AICc <2. 
...
recruit_ld_aic  #Null model is best
...
recruit_ts_aic$Model[5] <- "m5"
recruit_ts_aic$LL[5] <- logLik(recruit_g5.1t)
...
recruit_ts_aic  #Model 1 is best but null model ranked within AICc <2.
...
recruit_ls_aic$AICc[5] <- AICc(recruit_g5.1l)
recruit_ls_aic$Model[5] <- "m4"
recruit_ls_aic$LL[5] <- logLik(recruit_g5.1l)
recruit_ls_aic$AICc[6] <- AICc(recruit_g6.1l)
recruit_ls_aic$Model[6] <- "m5"
recruit_ls_aic$LL[6] <- logLik(recruit_g6.1l)
...
recruit_ls_aic  #Null model is best
...
recruit_tsc_aic  #Model 1 and 2 are best and no others ranked within AICc <2.
...
recruit_lsc_aic  #Null model is best

# Predict for proportion recruits ----
# NUll model best for littoralis. So no effect of time since fire on recruitment of any type.
nrecruit_gm1 <- data.frame(TSF = seq(min(tor_transects$TSF), max(tor_transects$TSF), length = 50))
nrecruit_gm2l <- expand.grid(TSF = min(tor_transects$TSF),
                             Latitude = seq(min(tor_transects$Latitude), max(tor_transects$Latitude), length = 50))
nrecruit_gm2a <- expand.grid(TSF = mean(tor_transects$TSF),
                             Latitude = seq(min(tor_transects$Latitude), max(tor_transects$Latitude), length = 50))
nrecruit_gm2h <- expand.grid(TSF = max(tor_transects$TSF),
                             Latitude = seq(min(tor_transects$Latitude), max(tor_transects$Latitude), length = 50))
...
# GLMER
nrecruit_m2l <-  expand.grid(r_TSF = min(tor_transects$r_TSF),
                         r_Latitude = seq(min(tor_transects$r_Latitude), max(tor_transects$r_Latitude), length = 50))
nrecruit_m2a <-  expand.grid(r_TSF = mean(tor_transects$r_TSF),
                         r_Latitude = seq(min(tor_transects$r_Latitude), max(tor_transects$r_Latitude), length = 50))
nrecruit_m2h <-  expand.grid(r_TSF = max(tor_transects$r_TSF),
                         r_Latitude = seq(min(tor_transects$r_Latitude), max(tor_transects$r_Latitude), length = 50))


precruit_m2l <- predictSE(recruit_m2t, newdata = nrecruit_m2l, se.fit = T, type = 'link')
nrecruit_m2l$fit.link <- precruit_m2l$fit
nrecruit_m2l$se.link <- precruit_m2l$se.fit
nrecruit_m2l$lci.link <- nrecruit_m2l$fit.link - (nrecruit_m2l$se.link * 1.96)
nrecruit_m2l$uci.link <- nrecruit_m2l$fit.link + (nrecruit_m2l$se.link * 1.96)
nrecruit_m2l$fit <- invlogit(nrecruit_m2l$fit.link)
nrecruit_m2l$se <- invlogit(nrecruit_m2l$se.link)
nrecruit_m2l$lci <- invlogit(nrecruit_m2l$lci.link)
nrecruit_m2l$uci <- invlogit(nrecruit_m2l$uci.link)

precruit_m2a <- predictSE(recruit_m2t, newdata = nrecruit_m2a, se.fit = T, type = 'link')
nrecruit_m2a$fit.link <- precruit_m2a$fit
nrecruit_m2a$se.link <- precruit_m2a$se.fit
nrecruit_m2a$lci.link <- nrecruit_m2a$fit.link - (nrecruit_m2a$se.link * 1.96)
nrecruit_m2a$uci.link <- nrecruit_m2a$fit.link + (nrecruit_m2a$se.link * 1.96)
nrecruit_m2a$fit <- invlogit(nrecruit_m2a$fit.link)
nrecruit_m2a$se <- invlogit(nrecruit_m2a$se.link)
nrecruit_m2a$lci <- invlogit(nrecruit_m2a$lci.link)
nrecruit_m2a$uci <- invlogit(nrecruit_m2a$uci.link)


precruit_m2h <- predictSE(recruit_m2t, newdata = nrecruit_m2h, se.fit = T, type = 'link')
nrecruit_m2h$fit.link <- precruit_m2h$fit
nrecruit_m2h$se.link <- precruit_m2h$se.fit
nrecruit_m2h$lci.link <- nrecruit_m2h$fit.link - (nrecruit_m2h$se.link * 1.96)
nrecruit_m2h$uci.link <- nrecruit_m2h$fit.link + (nrecruit_m2h$se.link * 1.96)
nrecruit_m2h$fit <- invlogit(nrecruit_m2h$fit.link)
nrecruit_m2h$se <- invlogit(nrecruit_m2h$se.link)
nrecruit_m2h$lci <- invlogit(nrecruit_m2h$lci.link)
nrecruit_m2h$uci <- invlogit(nrecruit_m2h$uci.link)


# GAM defaults
nrecruit_g1 <- nrecruit_gm1
precruit_g1 <- predict(recruit_g1t, newdata = nrecruit_gm1, se.fit = T, type = 'response')
nrecruit_g1$fit.link <- precruit_g1$fit
nrecruit_g1$se.link <- precruit_g1$se.fit
nrecruit_g1$lci.link <- nrecruit_g1$fit.link - (nrecruit_g1$se.link * 1.96)
nrecruit_g1$uci.link <- nrecruit_g1$fit.link + (nrecruit_g1$se.link * 1.96)
nrecruit_g1$fit <- invlogit(nrecruit_g1$fit.link)
nrecruit_g1$se <- invlogit(nrecruit_g1$se.link)
nrecruit_g1$lci <- invlogit(nrecruit_g1$lci.link)
nrecruit_g1$uci <- invlogit(nrecruit_g1$uci.link)

nrecruit_g2l <- nrecruit_gm2l
precruit_g2l <- predict(recruit_g2t, newdata = nrecruit_gm2l, se.fit = T, type = 'response')
nrecruit_g2l$fit.link <- precruit_g2l$fit
nrecruit_g2l$se.link <- precruit_g2l$se.fit
nrecruit_g2l$lci.link <- nrecruit_g2l$fit.link - (nrecruit_g2l$se.link * 1.96)
nrecruit_g2l$uci.link <- nrecruit_g2l$fit.link + (nrecruit_g2l$se.link * 1.96)
nrecruit_g2l$fit <- invlogit(nrecruit_g2l$fit.link)
nrecruit_g2l$se <- invlogit(nrecruit_g2l$se.link)
nrecruit_g2l$lci <- invlogit(nrecruit_g2l$lci.link)
nrecruit_g2l$uci <- invlogit(nrecruit_g2l$uci.link)


nrecruit_g2a <- nrecruit_gm2a
precruit_g2a <- predict(recruit_g2t, newdata = nrecruit_gm2a, se.fit = T, type = 'response')
nrecruit_g2a$fit.link <- precruit_g2a$fit
nrecruit_g2a$se.link <- precruit_g2a$se.fit
nrecruit_g2a$lci.link <- nrecruit_g2a$fit.link - (nrecruit_g2a$se.link * 1.96)
nrecruit_g2a$uci.link <- nrecruit_g2a$fit.link + (nrecruit_g2a$se.link * 1.96)
nrecruit_g2a$fit <- invlogit(nrecruit_g2a$fit.link)
nrecruit_g2a$se <- invlogit(nrecruit_g2a$se.link)
nrecruit_g2a$lci <- invlogit(nrecruit_g2a$lci.link)
nrecruit_g2a$uci <- invlogit(nrecruit_g2a$uci.link)


nrecruit_g2h <- nrecruit_gm2h
precruit_g2h <- predict(recruit_g2t, newdata = nrecruit_gm2h, se.fit = T, type = 'response')
nrecruit_g2h$fit.link <- precruit_g2h$fit
nrecruit_g2h$se.link <- precruit_g2h$se.fit
nrecruit_g2h$lci.link <- nrecruit_g2h$fit.link - (nrecruit_g2h$se.link * 1.96)
nrecruit_g2h$uci.link <- nrecruit_g2h$fit.link + (nrecruit_g2h$se.link * 1.96)
nrecruit_g2h$fit <- invlogit(nrecruit_g2h$fit.link)
nrecruit_g2h$se <- invlogit(nrecruit_g2h$se.link)
nrecruit_g2h$lci <- invlogit(nrecruit_g2h$lci.link)
nrecruit_g2h$uci <- invlogit(nrecruit_g2h$uci.link)


# GAM bs = cr k = 5 
nrecruit_g1.1 <- nrecruit_gm1
precruit_g1.1 <- predict(recruit_g1.1t, newdata = nrecruit_gm1, se.fit = T, type = 'response')
nrecruit_g1.1$fit.link <- precruit_g1.1$fit
nrecruit_g1.1$se.link <- precruit_g1.1$se.fit
nrecruit_g1.1$lci.link <- nrecruit_g1.1$fit.link - (nrecruit_g1.1$se.link * 1.96)
nrecruit_g1.1$uci.link <- nrecruit_g1.1$fit.link + (nrecruit_g1.1$se.link * 1.96)
nrecruit_g1.1$fit <- invlogit(nrecruit_g1.1$fit.link)
nrecruit_g1.1$se <- invlogit(nrecruit_g1.1$se.link)
nrecruit_g1.1$lci <- invlogit(nrecruit_g1.1$lci.link)
nrecruit_g1.1$uci <- invlogit(nrecruit_g1.1$uci.link)

nrecruit_g2.2l <- nrecruit_gm2l
precruit_g2.2l <- predict(recruit_g2.2t, newdata = nrecruit_gm2l, se.fit = T, type = 'response')
nrecruit_g2.2l$fit.link <- precruit_g2.2l$fit
nrecruit_g2.2l$se.link <- precruit_g2.2l$se.fit
nrecruit_g2.2l$lci.link <- nrecruit_g2.2l$fit.link - (nrecruit_g2.2l$se.link * 1.96)
nrecruit_g2.2l$uci.link <- nrecruit_g2.2l$fit.link + (nrecruit_g2.2l$se.link * 1.96)
nrecruit_g2.2l$fit <- invlogit(nrecruit_g2.2l$fit.link)
nrecruit_g2.2l$se <- invlogit(nrecruit_g2.2l$se.link)
nrecruit_g2.2l$lci <- invlogit(nrecruit_g2.2l$lci.link)
nrecruit_g2.2l$uci <- invlogit(nrecruit_g2.2l$uci.link)


nrecruit_g2.2a <- nrecruit_gm2a
precruit_g2.2a <- predict(recruit_g2.2t, newdata = nrecruit_gm2a, se.fit = T, type = 'response')
nrecruit_g2.2a$fit.link <- precruit_g2.2a$fit
nrecruit_g2.2a$se.link <- precruit_g2.2a$se.fit
nrecruit_g2.2a$lci.link <- nrecruit_g2.2a$fit.link - (nrecruit_g2.2a$se.link * 1.96)
nrecruit_g2.2a$uci.link <- nrecruit_g2.2a$fit.link + (nrecruit_g2.2a$se.link * 1.96)
nrecruit_g2.2a$fit <- invlogit(nrecruit_g2.2a$fit.link)
nrecruit_g2.2a$se <- invlogit(nrecruit_g2.2a$se.link)
nrecruit_g2.2a$lci <- invlogit(nrecruit_g2.2a$lci.link)
nrecruit_g2.2a$uci <- invlogit(nrecruit_g2.2a$uci.link)


nrecruit_g2.2h <- nrecruit_gm2h
precruit_g2.2h <- predict(recruit_g2.2t, newdata = nrecruit_gm2h, se.fit = T, type = 'response')
nrecruit_g2.2h$fit.link <- precruit_g2.2h$fit
nrecruit_g2.2h$se.link <- precruit_g2.2h$se.fit
nrecruit_g2.2h$lci.link <- nrecruit_g2.2h$fit.link - (nrecruit_g2.2h$se.link * 1.96)
nrecruit_g2.2h$uci.link <- nrecruit_g2.2h$fit.link + (nrecruit_g2.2h$se.link * 1.96)
nrecruit_g2.2h$fit <- invlogit(nrecruit_g2.2h$fit.link)
nrecruit_g2.2h$se <- invlogit(nrecruit_g2.2h$se.link)
nrecruit_g2.2h$lci <- invlogit(nrecruit_g2.2h$lci.link)
nrecruit_g2.2h$uci <- invlogit(nrecruit_g2.2h$uci.link)



# Plot predictions for proportion recruits -----
# GLMER
plot(nrecruit_m2l$r_Latitude, nrecruit_m2l$fit, type = 'l', las = 1, xlab = "", ylab = "", ylim = c(0,1), cex.axis = 1.4, col = "blue", xaxt = "n")
axis(side = 1, at = seq(-1.364717, 1.103191, 0.25), labels = round(seq(-28.28369, -27.31778, 0.1), 1))
mtext(side = 1, expression(bold("Latitude")), line = 4, cex = 1.5)
mtext(side = 2, expression(bold("Proportion saplings")), line = 3, cex = 1.5)
mtext(paste("AICc = ", round(AICc(recruit_m2t ),3), sep = ""), line = -1.5, cex = 1.2)
pg.ci(x = 'r_Latitude', data = "nrecruit_m2l", colour = rgb(0,0,1,0.1), lower = 'lci', upper = 'uci')
lines(nrecruit_m2a$r_Latitude, nrecruit_m2a$fit, type = "l", col = "black")
pg.ci(x = 'r_Latitude', data = "nrecruit_m2a", colour = rgb(0,0,0,0.1), lower = 'lci', upper = 'uci')
lines(nrecruit_m2h$r_Latitude, nrecruit_m2h$fit, type = "l", col = "red")
pg.ci(x = 'r_Latitude', data = "nrecruit_m2h", colour = rgb(1,0,0,0.1), lower = 'lci', upper = 'uci')

# GAM model 1 comparisons
dev.new(width = 16, height = 12, noRStudioGD = T, dpi = 300)
par(mfrow = c(2,2), mar = c(6,6,2,2), mgp = c(2.7,1,0), oma = c(0,0,0,10))

plot(nrecruit_g1$TSF, nrecruit_g1$fit, type = 'l', las = 1, xlab = "", ylab = "", ylim = c(0,1), cex.axis = 1.4)
mtext(side = 1, expression(bold("Time since fire")), line = 4, cex = 1.5)
mtext(side = 2, expression(bold("Proportion saplings")), line = 3, cex = 1.5)
mtext(paste("AICc = ", round(AICc(recruit_g1t ),3), sep = ""), line = -1.5, cex = 1.2)
pg.ci(x = 'TSF', data = "nrecruit_g1", colour = rgb(0,0,0,0.1), lower = 'lci', upper = 'uci')
mtext(expression(bold("(a) GAM defaults)")), cex = 2)


plot(nrecruit_g1.1$TSF, nrecruit_g1.1$fit, type = 'l', las = 1, xlab = "", ylab = "", ylim = c(0,1), cex.axis = 1.4)
mtext(side = 1, expression(bold("Time since fire")), line = 4, cex = 1.5)
mtext(side = 2, expression(bold("Proportion saplings")), line = 3, cex = 1.5)
mtext(paste("AICc = ", round(AICc(recruit_g1.1t ),3), sep = ""), line = -1.5, cex = 1.2)
pg.ci(x = 'TSF', data = "nrecruit_g1.1", colour = rgb(0,0,0,0.1), lower = 'lci', upper = 'uci')
mtext(expression(bold("(b) GAM bs = 'cr'")), cex = 2)

par(xpd = NA)
legend(x = 26, y = -0.5, legend = c("1 year", "9.5 years", "25 years"), col = c("blue", "black", 'red'), title = expression(bold("Time since \n fire")), lty = 1, lwd = 2, cex = 1.8, bty = "n")
par(xpd = F)

# No differences, select basis function that will be used for model 2.


# GAM model 2 comparisons
plot(nrecruit_g2l$Latitude, nrecruit_g2l$fit, type = 'l', las = 1, xlab = "", ylab = "", ylim = c(0,1), cex.axis = 1.4, col = "blue")
mtext(side = 1, expression(bold("Latitude")), line = 4, cex = 1.5)
mtext(side = 2, expression(bold("Proportion saplings")), line = 3, cex = 1.5)
mtext(paste("AICc = ", round(AICc(recruit_g2t ),3), sep = ""), line = -1.5, cex = 1.2)
pg.ci(x = 'Latitude', data = "nrecruit_g2l", colour = rgb(0,0,1,0.1), lower = 'lci', upper = 'uci')
lines(nrecruit_g2a$Latitude, nrecruit_g2a$fit, type = "l", col = "black")
pg.ci(x = 'Latitude', data = "nrecruit_g2a", colour = rgb(0,0,0,0.1), lower = 'lci', upper = 'uci')
lines(nrecruit_g2h$Latitude, nrecruit_g2h$fit, type = "l", col = "red")
pg.ci(x = 'Latitude', data = "nrecruit_g2h", colour = rgb(1,0,0,0.1), lower = 'lci', upper = 'uci')
mtext(expression(bold("(c) GAM defaults")), cex = 2)


plot(nrecruit_g2.2l$Latitude, nrecruit_g2.2l$fit, type = 'l', las = 1, xlab = "", ylab = "", ylim = c(0,1), cex.axis = 1.4, col = "blue")
mtext(side = 1, expression(bold("Latitude")), line = 4, cex = 1.5)
mtext(side = 2, expression(bold("Proportion saplings")), line = 3, cex = 1.5)
mtext(paste("AICc = ", round(AICc(recruit_g2.2t ),3), sep = ""), line = -1.5, cex = 1.2)
pg.ci(x = 'Latitude', data = "nrecruit_g2.2l", colour = rgb(0,0,1,0.1), lower = 'lci', upper = 'uci')
lines(nrecruit_g2.2a$Latitude, nrecruit_g2.2a$fit, type = "l", col = "black")
pg.ci(x = 'Latitude', data = "nrecruit_g2.2a", colour = rgb(0,0,0,0.1), lower = 'lci', upper = 'uci')
lines(nrecruit_g2.2h$Latitude, nrecruit_g2.2h$fit, type = "l", col = "red")
pg.ci(x = 'Latitude', data = "nrecruit_g2.2h", colour = rgb(1,0,0,0.1), lower = 'lci', upper = 'uci')
mtext(expression(bold("(d) GAM bs = 'cr'")), cex = 2)

# For both model 1 and model 2, changing the basis function does not produce different plots. There is also only a very minor difference in AICc for the model 2 with latitude which is the best model between these two basis functions. Need to determine if tp or cr would be better or do we choose based on AIC since there is no major differences?


# GLM and GAM
dev.new(width = 24, height = 6, noRStudioGD = T, dpi = 300)
par(mfrow = c(1,3), mar = c(6,6,3,2), mgp = c(2.7,1,0), oma = c(0,0,0,10))

plot(nrecruit_m2l$r_Latitude, nrecruit_m2l$fit, type = 'l', las = 1, xlab = "", ylab = "", ylim = c(0,1), cex.axis = 1.4, col = "blue", xaxt = "n")
axis(side = 1, at = seq(-1.364717, 1.103191, 0.25), labels = round(seq(-28.28369, -27.31778, 0.1), 1), cex.axis = 1.4)
mtext(side = 1, expression(bold("Latitude")), line = 4, cex = 1.5)
mtext(side = 2, expression(bold("Proportion saplings")), line = 3, cex = 1.5)
mtext(paste("AICc = ", round(AICc(recruit_m2t ),3), sep = ""), line = -1.5, cex = 1.2)
pg.ci(x = 'r_Latitude', data = "nrecruit_m2l", colour = rgb(0,0,1,0.1), lower = 'lci', upper = 'uci')
...
lines(nrecruit_m2h$r_Latitude, nrecruit_m2h$fit, type = "l", col = "red")
pg.ci(x = 'r_Latitude', data = "nrecruit_m2h", colour = rgb(1,0,0,0.1), lower = 'lci', upper = 'uci')
mtext(expression(bold("(a) GLMER")))

par(xpd = NA)
legend(x = 7.4, y = 1, legend = c("1 year", "9.5 years", "25 years"), col = c("blue", "black", 'red'), title = expression(bold("Time since \n fire")), lty = 1, lwd = 2, cex = 1.8, bty = "n")
par(xpd = F)
...
lines(nrecruit_g2.2h$Latitude, nrecruit_g2.2h$fit, type = "l", col = "red")
pg.ci(x = 'Latitude', data = "nrecruit_g2.2h", colour = rgb(1,0,0,0.1), lower = 'lci', upper = 'uci')
mtext(expression(bold("(c) GAM bs = 'cr'")), cex = 2)
...
tor_fecundity <- tor_tree[tor_tree$Cone_presence == "Y",]
lit_fecundity <- lit_tree[lit_tree$Cone_presence == "Y",]

tor_fecundity$r_TSF <- scale(tor_fecundity$TSF)
tor_fecundity$r_Latitude <- scale(tor_fecundity$Latitude)
tor_fecundity$r_height <- scale(tor_fecundity$height)
...
lit_fecundity$r_height <- scale(lit_fecundity$height)
...
fecundity_tnull <- glmer(Cone_number ~ 1 + (1 | Location/Transect), family = binomial, data = tor_fecundity)
fecundity_gt_null <- gam(Cone_number ~ 1, random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')

fecundity_lnull <- glmer(Cone_number ~ 1 + (1 | Location/Transect), family = binomial, data = lit_fecundity)
fecundity_gl_null <- gam(Cone_number ~ 1, random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')



fecundity_m1t <- glmer(Cone_number ~ r_TSF (1 | Location/Transect), family = binomial, data = tor_fecundity)
summary(fecundity_m1t)
fecundity_g1t <- gam(Cone_number ~ s(TSF), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g1t)
plot(fecundity_g1t)
fecundity_g1.1t <- gam(Cone_number ~ s(TSF, bs = 'cr', k = 5), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g1.1t)
plot(fecundity_g1.1t)

fecundity_m1l <- glmer(Cone_number ~ r_TSF + (1 | Location/Transect), family = binomial, data = lit_fecundity, control = glmerControl(optCtrl=list(maxfun=30)))
summary(fecundity_m1l) # Not a very good model
fecundity_g1l <- gam(Cone_number ~ s(TSF, k = 4), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g1l)
plot(fecundity_g1l)
fecundity_g1.1l <- gam(Cone_number ~ s(TSF, bs = 'cr', k = 4), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g1.1l)
plot(fecundity_g1.1l)


fecundity_m2t <- glmer(Cone_number ~ r_TSF * r_Latitude + (1|Location/Transect), family = binomial, data = tor_fecundity)
summary(fecundity_m2t)
fecundity_g2t <- gam(Cone_number ~ s(TSF) + s(Latitude) + ti(Latitude, by = TSF), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g2t)
plot(fecundity_g2t)
fecundity_g2.1t <- gam(Cone_number ~ s(TSF) + s(Latitude) + ti(Latitude, by = TSF, bs = 'tp', k = 10), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g2.1t)
plot(fecundity_g2.1t)
fecundity_g2.2t <- gam(Cone_number ~ s(TSF, bs = 'cr', k = 5) + s(Latitude, bs = 'cr', k = 5) + ti(Latitude, by = TSF, bs = 'cr', k = 5), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g2.1t)
plot(fecundity_g2.2t)


#fecundity_m2l <- glmer(Cone_number ~ TSF * Latitude + (1|Location/Transect), family = binomial, data = lit_fecundity) # This model fails due to the low number of points available to model an interaction. We can only model an additive effect of TSF and latitude
fecundity_m2.1l <- glmer(Cone_number ~ r_TSF + r_Latitude + (1|Location/Transect), family = binomial, data = lit_fecundity, control = glmerControl(optCtrl=list(maxfun=30)))
summary(fecundity_m2.1l) # Not very good model
fecundity_g2l <- gam(Cone_number ~ s(TSF, k = 4) + s(Latitude, k = 4) + ti(Latitude, by = TSF), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g2l)
plot(fecundity_g2l)
fecundity_g2.1l <- gam(Cone_number ~ s(TSF, k = 4) + s(Latitude, k = 4) + ti(Latitude, by = TSF, bs = 'tp', k = 8), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g2.1l)
plot(fecundity_g2.1l)
fecundity_g2.2l <- gam(Cone_number ~ s(TSF, bs = 'cr', k = 4) + s(Latitude, bs = 'cr', k = 5) + ti(Latitude, by = TSF, bs = 'cr', k = 5), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g2.1l)
plot(fecundity_g2.2l)



fecundity_m3t <- glmer(Cone_number ~ r_TSF * r_FPC + (1|Location/Transect), family = binomial, data = tor_fecundity)
fecundity_g3t <- gam(Cone_number ~ s(TSF) + s(FPC) + ti(FPC, by = TSF), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g3t)
plot(fecundity_g3t)
fecundity_g3.1t <- gam(Cone_number ~ s(TSF) + s(FPC) + ti(FPC, by = TSF, bs = 'tp', k = 10), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g3.1t)
plot(fecundity_g3.1t)
fecundity_g3.2t <- gam(Cone_number ~ s(TSF, bs = 'cr', k = 5) + s(FPC, bs = 'cr', k = 5) + ti(FPC, by = TSF, bs = 'cr', k = 5), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g3.1t)
plot(fecundity_g3.2t)


fecundity_m3l <- glmer(Cone_number ~ r_TSF * r_FPC + (1|Location/Transect), family = binomial, data = lit_fecundity, control = glmerControl(optCtrl=list(maxfun=30)))
summary(fecundity_m3l) # Not a very good model
fecundity_g3l <- gam(Cone_number ~ s(TSF, k = 4) + s(FPC, k = 4) + ti(FPC, by = TSF), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g3l)
plot(fecundity_g3l)
fecundity_g3.1l <- gam(Cone_number ~ s(TSF, k = 4) + s(FPC, k = 4) + ti(FPC, by = TSF, bs = 'tp', k = 7), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g3.1l)
plot(fecundity_g3.1l)
fecundity_g3.2l <- gam(Cone_number ~ s(TSF, bs = 'cr', k = 4) + s(FPC, bs = 'cr', k = 5) + ti(FPC, by = TSF, bs = 'cr', k = 5), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g3.2l)
plot(fecundity_g3.2l)


fecundity_m4t <- glmer(Cone_number ~ r_TSF * r_Precip + (1|Location/Transect), family = binomial, data = tor_fecundity)
fecundity_g4t <- gam(Cone_number ~ s(TSF) + s(Precip) + ti(Precip, by = TSF), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g4t)
plot(fecundity_g4t)
fecundity_g4.1t <- gam(Cone_number ~ s(TSF) + s(Precip) + ti(Precip, by = TSF, bs = 'tp', k = 10), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g4.1t)
plot(fecundity_g4.1t)
fecundity_g4.2t <- gam(Cone_number ~ s(TSF, bs = 'cr', k = 5) + s(Precip, bs = 'cr', k = 5) + ti(Precip, by = TSF, bs = 'cr', k = 5), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g4.1t)
plot(fecundity_g4.2t)


fecundity_m4l <- glmer(Cone_number ~ r_TSF * r_Precip + (1|Location/Transect), family = binomial, data = lit_fecundity, control = glmerControl(optCtrl=list(maxfun=30)))
summary(fecundity_m4l) # Not a very good model
fecundity_g4l <- gam(Cone_number ~ s(TSF, k = 4) + s(Precip, k = 4) + ti(Precip, by = TSF), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g4l)
plot(fecundity_g4l)
fecundity_g4.1l <- gam(Cone_number ~ s(TSF, k = 4) + s(Precip, k = 4) + ti(Precip, by = TSF, bs = 'tp', k = 7), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g4.1l)
plot(fecundity_g4.1l)
fecundity_g4.2l <- gam(Cone_number ~ s(TSF, bs = 'cr', k = 4) + s(Precip, bs = 'cr', k = 5) + ti(Precip, by = TSF, bs = 'cr', k = 5), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g4.2l)
plot(fecundity_g4.2l)



fecundity_m5t <- glmer(Cone_number ~ r_TSF * r_Precip + (1|Location/Transect), family = binomial, data = tor_fecundity)
fecundity_g5t <- gam(Cone_number ~ s(TSF) + s(Precip) + ti(Precip, by = TSF), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g5t)
plot(fecundity_g5t)
fecundity_g5.1t <- gam(Cone_number ~ s(TSF) + s(Precip) + ti(Precip, by = TSF, bs = 'tp', k = 10), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g5.1t)
plot(fecundity_g5.1t)
fecundity_g5.2t <- gam(Cone_number ~ s(TSF, bs = 'cr', k = 5) + s(Precip, bs = 'cr', k = 5) + ti(Precip, by = TSF, bs = 'cr', k = 5), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g5.1t)
plot(fecundity_g5.2t)


fecundity_m5l <- glmer(Cone_number ~ r_TSF * r_Precip + (1|Location/Transect), family = binomial, data = lit_fecundity, control = glmerControl(optCtrl=list(maxfun=30)))
summary(fecundity_m5l) # Not a very good model
fecundity_g5l <- gam(Cone_number ~ s(TSF, k = 4) + s(Precip, k = 4) + ti(Precip, by = TSF), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g5l)
plot(fecundity_g5l)
fecundity_g5.1l <- gam(Cone_number ~ s(TSF, k = 4) + s(Precip, k = 4) + ti(Precip, by = TSF, bs = 'tp', k = 7), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g5.1l)
plot(fecundity_g5.1l)
fecundity_g5.2l <- gam(Cone_number ~ s(TSF, bs = 'cr', k = 4) + s(Precip, bs = 'cr', k = 5) + ti(Precip, by = TSF, bs = 'cr', k = 5), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
...
fecundity_m6t <- glmer(Cone_number ~ r_TSF * r_Precip + (1|Location/Transect), family = binomial, data = tor_fecundity)
fecundity_g6t <- gam(Cone_number ~ s(TSF) + s(Precip) + ti(Precip, by = TSF), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g6t)
plot(fecundity_g6t)
fecundity_g6.1t <- gam(Cone_number ~ s(TSF) + s(Precip) + ti(Precip, by = TSF, bs = 'tp', k = 10), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g6.1t)
plot(fecundity_g6.1t)
fecundity_g6.2t <- gam(Cone_number ~ s(TSF, bs = 'cr', k = 5) + s(Precip, bs = 'cr', k = 5) + ti(Precip, by = TSF, bs = 'cr', k = 5), random = ~ (1 | Transect/Location), family = binomial, data = tor_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g6.1t)
plot(fecundity_g6.2t)


fecundity_m6l <- glmer(Cone_number ~ r_TSF * r_Precip + (1|Location/Transect), family = binomial, data = lit_fecundity, control = glmerControl(optCtrl=list(maxfun=30)))
summary(fecundity_m6l) # Not a very good model
fecundity_g6l <- gam(Cone_number ~ s(TSF, k = 4) + s(Precip, k = 4) + ti(Precip, by = TSF), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g6l)
plot(fecundity_g6l)
fecundity_g6.1l <- gam(Cone_number ~ s(TSF, k = 4) + s(Precip, k = 4) + ti(Precip, by = TSF, bs = 'tp', k = 7), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
par(mfrow = c(2,2)); gam.check(fecundity_g6.1l)
plot(fecundity_g6.1l)
fecundity_g6.2l <- gam(Cone_number ~ s(TSF, bs = 'cr', k = 4) + s(Precip, bs = 'cr', k = 5) + ti(Precip, by = TSF, bs = 'cr', k = 5), random = ~ (1 | Transect/Location), family = binomial, data = lit_fecundity, method = 'ML')
...
aictab(seed_ff_aic_t) # Null model is best
...
# GAM ti(bs='cr')
seed_ff_tscc_aic <- as.data.frame(1:6)
...
aictab(sap_ff_aic_t) # Null model is best
...
sap_ff_td_aic  # Null model is best
...
sap_ff_ts_aic  #Null model is best.
...
sap_ff_tsc_aic  #Model 1 and 2 are best but null model is ranked within AICc <2.
...
# Run GLMER without nested transect affect
sap_ff_tnull <- glmer(Proportion_saplings ~ 1 + (1 | Location), family = binomial, data = tor_transects)
sap_ff_lnull <- glmer(Proportion_saplings ~ 1 + (1 | Location), family = binomial, data = lit_transects)


sap_ff_m1t <- glmer(Proportion_saplings ~ r_Fire_freq + (1 | Location), family = binomial, data = tor_transects)
sap_ff_m1l <- glmer(Proportion_saplings ~ r_Fire_freq + (1 | Location), family = binomial, data = lit_transects, control = glmerControl(optCtrl=list(maxfun=30)))


sap_ff_m2t <- glmer(Proportion_saplings ~ r_Fire_freq * r_Latitude + (1|Location), family = binomial, data = tor_transects)
#sap_ff_m2l <- glmer(Proportion_saplings ~ Fire_freq * Latitude + (1|Location), family = binomial, data = lit_transects) # This model fails due to the low number of points available to model an interaction. We can only model an additive effect of Fire_freq and latitude
sap_ff_m2.1l <- glmer(Proportion_saplings ~ r_Fire_freq + r_Latitude + (1|Location), family = binomial, data = lit_transects)

sap_ff_m3t <- glmer(Proportion_saplings ~ r_Fire_freq * r_FPC + (1|Location), family = binomial, data = tor_transects)
sap_ff_m3l <- glmer(Proportion_saplings ~ r_Fire_freq * r_FPC + (1|Location), family = binomial, data = lit_transects)


sap_ff_aic_t <- list(sap_ff_tnull, sap_ff_m1t, sap_ff_m2t, sap_ff_m3t)
aictab(sap_ff_aic_t) # Null model is best
sap_ff_aic_l <- list(sap_ff_lnull, sap_ff_m1l, sap_ff_m2.1l, sap_ff_m3l)
aictab(sap_ff_aic_l) # Model 2 is good but null model is quite close. This is also an additive only model

# Predict from littoralis model2.1
sap_fire <- data.frame(r_Fire_freq = seq(min(lit_transects$r_Fire_freq), max(lit_transects$r_Fire_freq), length = 50),
                       r_Latitude = mean(lit_transects$r_Latitude))
sap_latitude <- data.frame(r_Fire_freq = mean(lit_transects$r_Fire_freq),
                           r_Latitude = seq(min(lit_transects$r_Latitude), max(lit_transects$r_Latitude), length = 50))

sfpred <- predictSE(sap_ff_m2.1l, newdata = sap_fire, se.fit = T, type = 'link')
sap_fire$fit.link <- sfpred$fit
sap_fire$se.link <- sfpred$se.fit
sap_fire$lci.link <- sap_fire$fit.link - (sap_fire$se.link * 1.96)
sap_fire$uci.link <- sap_fire$fit.link + (sap_fire$se.link * 1.96)
sap_fire$fit <- invlogit(sap_fire$fit.link)
sap_fire$se <- invlogit(sap_fire$se.link)
sap_fire$lci <- invlogit(sap_fire$lci.link)
sap_fire$uci <- invlogit(sap_fire$uci.link)


slpred <- predictSE(sap_ff_m2.1l, newdata = sap_latitude, se.fit = T, type = 'link')
sap_latitude$fit.link <- slpred$fit
sap_latitude$se.link <- slpred$se.fit
sap_latitude$lci.link <- sap_latitude$fit.link - (sap_latitude$se.link * 1.96)
sap_latitude$uci.link <- sap_latitude$fit.link + (sap_latitude$se.link * 1.96)
sap_latitude$fit <- invlogit(sap_latitude$fit.link)
sap_latitude$se <- invlogit(sap_latitude$se.link)
sap_latitude$lci <- invlogit(sap_latitude$lci.link)
sap_latitude$uci <- invlogit(sap_latitude$uci.link)



par(mfrow = c(1,2))
plot(sap_fire$r_Fire_freq, sap_fire$fit, type = 'l')
plot(sap_latitude$r_Latitude, sap_latitude$fit, type = 'l')
# The plot with latitude alone is not useful. Not really explaining anything when we removed the nested location/transect random effect to just be location.
...
recff_m4l <- glmer(Proportion_recruits ~ r_Fire_freq * r_Precip + (1|Location/Transect), family = binomial, data = lit_transects, control = glmerControl(optCtrl=list(maxfun=30)))
...
recff_m5l <- glmer(Proportion_recruits ~ r_Fire_freq * r_Temp + (1|Location/Transect), family = binomial, data = lit_transects, control = glmerControl(optCtrl=list(maxfun=30)))
...
aictab(recff_aic_t) # Null model is best
...
recff_td_aic  # Null model is best
...
recff_ld_aic$LL[5] <- logLik(recff_g5l)
...
recff_ld_aic  #Null model is best
...
recff_ts_aic  # Null model is best
...
recff_ls_aic  #Null model is best
...
recff_tsc_aic  # Null model is best
...
recff_lsc_aic  #Null model is best
...
# 6.3 Seed size ----
seedwt_t_null <- glmer(seed_wt_mg ~ 1 + (1|Individual), family = Gamma, data = tor_cum.prop)
seedwt_t_gnull <- gam(seed_wt_mg ~ 1, random = ~ (1|Individual), family = Gamma, data = tor_cum.prop, method = 'ML')

seedwt_l_null <- glmer(seed_wt_mg ~ 1 + (1|Individual), family = Gamma, data = lit_cum.prop)
seedwt_l_gnull <- gam(seed_wt_mg ~ 1, random = ~ (1|Individual), family = Gamma, data = lit_cum.prop, method = 'ML')



seedwt_m1t <- glmer(seed_wt_mg ~ r_fire_freq + (1|Individual), family = Gamma, data = tor_cum.prop)
summary(seedwt_m1t)
seedwt_g1t <- gam(seed_wt_mg ~ s(fire_freq, k = 8), random = ~ (1|Individual), family = Gamma, data = tor_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g1t)
plot(seedwt_g1t)
seedwt_g1.1t <- gam(seed_wt_mg ~ s(fire_freq, bs = 'cr', k = 5), random = ~ (1|Individual) family = Gamma, data = tor_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g1.1t)
plot(seedwt_g1.1t)

seedwt_m1l <- glmer(seed_wt_mg ~ r_fire_freq + (1|Individual), family = Gamma, data = lit_cum.prop)
summary(seedwt_m1l)
seedwt_g1l <- gam(seed_wt_mg ~ s(fire_freq, k = 3), random = ~ (1|Individual), family = Gamma, data = lit_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g1l)
plot(seedwt_g1l)
seedwt_g1.1l <- gam(seed_wt_mg ~ s(fire_freq, bs = 'cr', k = 3), random = ~ (1|Individual), family = Gamma, data = lit_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g1.1l)
plot(seedwt_g1.1l)


seedwt_m2t <- glmer(seed_wt_mg ~ r_fire_freq * r_Latitude + (1|Individual), family = Gamma, data = tor_cum.prop)
summary(seedwt_m2t)
seedwt_g2t <- gam(seed_wt_mg ~ s(fire_freq, k = 8) + s(Latitude) + ti(Latitude, by = fire_freq), random = ~ (1|Individual), family = Gamma, data = tor_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g2t)
plot(seedwt_g2t)
summary(seedwt_g2t)
seedwt_g2.1t <- gam(seed_wt_mg ~ s(fire_freq, k = 8) + s(Latitude) + ti(Latitude, by = fire_freq, bs = 'tp', k = 10), random = ~ (1|Individual), family = Gamma, data = tor_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g2.1t)
plot(seedwt_g2.1t)
seedwt_g2.2t <- gam(seed_wt_mg ~ s(fire_freq, bs = 'cr', k = 5) + s(Latitude, bs = 'cr', k = 5) + ti(Latitude, by = fire_freq, bs = 'cr', k = 5), random = ~ (1|Individual), family = Gamma, data = tor_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g2.1t)
plot(seedwt_g2.2t)


seedwt_m2l <- glmer(seed_wt_mg ~ fire_freq * Latitude + (1|Individual), family = Gamma, data = lit_cum.prop) # This model fails due to the low number of points available to model an interaction. We can only model an additive effect of fire_freq and latitude
summary(seedwt_m2l)
seedwt_g2l <- gam(seed_wt_mg ~ s(fire_freq, k = 3) + s(Latitude, k = 8) + ti(Latitude, by = fire_freq), random = ~ (1|Individual), family = Gamma, data = lit_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g2l)
plot(seedwt_g2l)
seedwt_g2.1l <- gam(seed_wt_mg ~ s(fire_freq, k = 3) + s(Latitude, k = 8) + ti(Latitude, by = fire_freq, bs = 'tp', k = 8), random = ~ (1|Individual), family = Gamma, data = lit_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g2.1l)
plot(seedwt_g2.1l)
seedwt_g2.2l <- gam(seed_wt_mg ~ s(fire_freq, bs = 'cr', k = 3) + s(Latitude, bs = 'cr', k = 5) + ti(Latitude, by = fire_freq, bs = 'cr', k = 5), random = ~ (1|Individual), family = Gamma, data = lit_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g2.1l)
plot(seedwt_g2.2l)



seedwt_m3t <- glmer(seed_wt_mg ~ r_fire_freq * r_FPC + (1|Individual), family = Gamma, data = tor_cum.prop)
summary(seedwt_m3t)
seedwt_g3t <- gam(seed_wt_mg ~ s(fire_freq, k = 8) + s(FPC) + ti(FPC, by = fire_freq), random = ~ (1|Individual), family = Gamma, data = tor_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g3t)
plot(seedwt_g3t)
seedwt_g3.1t <- gam(seed_wt_mg ~ s(fire_freq, k = 8) + s(FPC) + ti(FPC, by = fire_freq, bs = 'tp', k = 10), random = ~ (1|Individual), family = Gamma, data = tor_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g3.1l)
plot(seedwt_g3.1t)
seedwt_g3.2t <- gam(seed_wt_mg ~ s(fire_freq, bs = 'cr', k = 5) + s(FPC, bs = 'cr', k = 5) + ti(FPC, by = fire_freq, bs = 'cr', k = 5), random = ~ (1|Individual), family = Gamma, data = tor_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g3.1l)
plot(seedwt_g3.2t)


seedwt_m3l <- glmer(seed_wt_mg ~ r_fire_freq * r_FPC + (1|Individual), family = Gamma, data = lit_cum.prop)
summary(seedwt_m3l)
seedwt_g3l <- gam(seed_wt_mg ~ s(fire_freq, k = 3) + s(FPC, k = 7) + ti(FPC, by = fire_freq), random = ~ (1|Individual), family = Gamma, data = lit_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g3l)
plot(seedwt_g3l)
seedwt_g3.1l <- gam(seed_wt_mg ~ s(fire_freq, k = 3) + s(FPC, k = 7) + ti(FPC, by = fire_freq, bs = 'tp', k = 7), random = ~ (1|Individual), family = Gamma, data = lit_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g3.1l)
plot(seedwt_g3.1l)
seedwt_g3.2l <- gam(seed_wt_mg ~ s(fire_freq, bs = 'cr', k = 3) + s(FPC, bs = 'cr', k = 5) + ti(FPC, by = fire_freq, bs = 'cr', k = 5), random = ~ (1|Individual), family = Gamma, data = lit_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g3.2l)
plot(seedwt_g3.2l)


seedwt_m4t <- glmer(seed_wt_mg ~ r_fire_freq * r_Precip + (1|Individual), family = Gamma, data = tor_cum.prop)
summary(seedwt_m4t)
seedwt_g4t <- gam(seed_wt_mg ~ s(fire_freq, k = 8) + s(Precip) + ti(Precip, by = fire_freq), random = ~ (1|Individual), family = Gamma, data = tor_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g4t)
plot(seedwt_g4t)
seedwt_g4.1t <- gam(seed_wt_mg ~ s(fire_freq, k = 8) + s(Precip) + ti(Precip, by = fire_freq, bs = 'tp', k = 10), random = ~ (1|Individual), family = Gamma, data = tor_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g4.1l)
plot(seedwt_g4.1t)
seedwt_g4.2t <- gam(seed_wt_mg ~ s(fire_freq, bs = 'cr', k = 5) + s(Precip, bs = 'cr', k = 5) + ti(Precip, by = fire_freq, bs = 'cr', k = 5), random = ~ (1|Individual), family = Gamma, data = tor_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g4.1l)
plot(seedwt_g4.2t)


seedwt_m4l <- glmer(seed_wt_mg ~ r_fire_freq * r_Precip + (1|Individual), family = Gamma, data = lit_cum.prop)
summary(seedwt_m4l)
seedwt_g4l <- gam(seed_wt_mg ~ s(fire_freq, k = 3) + s(Precip, k = 7) + ti(Precip, by = fire_freq), random = ~ (1|Individual), family = Gamma, data = lit_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g4l)
plot(seedwt_g4l)
seedwt_g4.1l <- gam(seed_wt_mg ~ s(fire_freq, k = 3) + s(Precip, k = 7) + ti(Precip, by = fire_freq, bs = 'tp', k = 7), random = ~ (1|Individual), family = Gamma, data = lit_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g4.1l)
plot(seedwt_g4.1l)
seedwt_g4.2l <- gam(seed_wt_mg ~ s(fire_freq, bs = 'cr', k = 3) + s(Precip, bs = 'cr', k = 5) + ti(Precip, by = fire_freq, bs = 'cr', k = 5), random = ~ (1|Individual), family = Gamma, data = lit_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g4.2l)
plot(seedwt_g4.2l)


seedwt_m5t <- glmer(seed_wt_mg ~ r_fire_freq * r_Temp + (1|Individual), family = Gamma, data = tor_cum.prop)
summary(seedwt_m5t)
seedwt_g5t <- gam(seed_wt_mg ~ s(fire_freq, k = 8) + s(Temp) + ti(Temp, by = fire_freq), random = ~ (1|Individual), family = Gamma, data = tor_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g5t)
plot(seedwt_g5t)
seedwt_g5.1t <- gam(seed_wt_mg ~ s(fire_freq, k = 8) + s(Temp) + ti(Temp, by = fire_freq, bs = 'tp', k = 10), random = ~ (1|Individual), family = Gamma, data = tor_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g5.1l)
plot(seedwt_g5.1t)
seedwt_g5.2t <- gam(seed_wt_mg ~ s(fire_freq, bs = 'cr', k = 5) + s(Temp, bs = 'cr', k = 5) + ti(Temp, by = fire_freq, bs = 'cr', k = 5), random = ~ (1|Individual), family = Gamma, data = tor_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g5.1l)
plot(seedwt_g5.2t)


seedwt_m5l <- glmer(seed_wt_mg ~ r_fire_freq * r_Temp + (1|Individual), family = Gamma, data = lit_cum.prop)
summary(seedwt_m5l)
seedwt_g5l <- gam(seed_wt_mg ~ s(fire_freq, k = 3) + s(Temp, k = 7) + ti(Temp, by = fire_freq), random = ~ (1|Individual), family = Gamma, data = lit_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g5l)
plot(seedwt_g5l)
seedwt_g5.1l <- gam(seed_wt_mg ~ s(fire_freq, k = 3) + s(Temp, k = 7) + ti(Temp, by = fire_freq, bs = 'tp', k = 7), random = ~ (1|Individual), family = Gamma, data = lit_cum.prop, method = 'ML')
par(mfrow = c(2,2)); gam.check(seedwt_g5.1l)
plot(seedwt_g5.1l)
seedwt_g5.2l <- gam(seed_wt_mg ~ s(fire_freq, bs = 'cr', k = 3) + s(Temp, bs = 'cr', k = 5) + ti(Temp, by = fire_freq, bs = 'cr', k = 5), random = ~ (1|Individual), family = Gamma, data = lit_cum.prop, method = 'ML')
...
seedwt_td_aic  #Null model is best
...
seedwt_ld_aic  #Null model is best
...
seedwt_ts_aic$AICc[2] <- AICc(seedwt_g1.1t)
seedwt_ts_aic$Model[2] <- "m1"
seedwt_ts_aic$LL[2] <- logLik(seedwt_g1.1t)
...
seedwt_ts_aic  #Null model is best
...
seedwt_ls_aic$AICc[2] <- AICc(seedwt_g1.1l)
seedwt_ls_aic$Model[2] <- "m1"
seedwt_ls_aic$LL[2] <- logLik(seedwt_g1.1l)
...
seedwt_ls_aic  #Null model is best
...
seedwt_tsc_aic$AICc[2] <- AICc(seedwt_g1t)
seedwt_tsc_aic$Model[2] <- "m1"
seedwt_tsc_aic$LL[2] <- logLik(seedwt_g1t)
...
seedwt_tsc_aic  #Null model is best

seedwt_lscc_aic <- as.data.frame(1:4)
...
seedwt_lsc_aic$AICc[2] <- AICc(seedwt_g1l)
seedwt_lsc_aic$Model[2] <- "m1"
seedwt_lsc_aic$LL[2] <- logLik(seedwt_g1l)
...
seedwt_lsc_aic  #Null model is best
